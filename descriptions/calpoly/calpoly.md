# Cal Poly SLO

Computer Engineering at California Polytechnic State University, San Luis Obispo
(B.S., class of 2029). This folder collects coursework — the assignments, labs,
and projects from each class — alongside the occasional cross-disciplinary
project.

## What's in here

- **csc-101** — Fundamentals of Computer Science.
- **csc-202** — Data Structures.
- **csc-203** — Project-Based Object-Oriented Programming: labs and projects,
  including a grid-based Rock Paper Scissors simulation and a weekly-calendar
  event system.
- **cpe-225** — Computer Organization: C and RISC-V assembly assignments.
- **generational-wealth-inequality** — a one-page project site built for ENGL
  133, on generational wealth inequality in California.

Open a folder to browse its assignments, or open a file to read the writeup.
